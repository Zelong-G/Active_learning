...
def main(parser_data):
    ...
    with torch.no_grad():
        result_list = []
        for image, targets in val_dataset_loader:
            # 将图片传入指定设备device
            image = list(img.to(device) for img in image)
            ...
            # 解析预测结果和真实标签（仅选取置信度最高的预测类别）
            for i, output in enumerate(outputs):
                if len(output["labels"]) > 0:
                    pred_labels = output["labels"].tolist()  # 预测的类别ID
                    pred_scores = output["scores"].tolist()  # 预测的置信度
                    ...
                    # 真实类别（可能有多个）
                    true_labels = targets[i]["labels"].tolist()

                    # 映射类别ID到名称
                    best_label_name = category_index[str(best_label)]
                    true_labels_names = [category_index[str(lbl)] for lbl in true_labels]

                    # 打印信息
                    # print(f"Image {i}:")
                    print(f"  True Labels: {true_labels_names}, Predicted Label: {best_label_name} (Score: {best_score:.3f})")
                    #
                    # 记录信息
                    result_list.append({
                        "image_id": targets[i]["image_id"].item(),
                        "true_labels": true_labels_names,
                        "pred_label": best_label_name,
                        "score": best_score
                    })
                else:
                    print(f"Image {i}: No valid predictions.")
                    result_list.append({
                        "image_id": targets[i]["image_id"].item(),
                        "true_labels": [],
                        "pred_label": "None",
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default=r'D:\work\Active_learning\mask_rcnn\WBC_Dataset\active-learning-4-cell3', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./run_result/active3/11/best.pth', type=str, help='training weights')
