...
if __name__ == "__main__":
    ...
    for j in range (8,11):
        for i in range (1,12):
            ...
            9. 训练
            '''
            data_folder = "/active_learning0"+str(j)
            output_folder = "./RBC_Dataset"+data_folder+"/result/"+str(i)
            if i == 1:
                #1. 数据划分
                from RBC.get_annotation import create_coco_json,split
                data_dir = r'./RBC_Dataset\all'
                output_dir = r'./RBC_Dataset'+data_folder
                label_dir = r'./RBC_Dataset\COCO_Annotations_png'
                # 设置图片路径和目标文件夹

                split(data_dir, output_dir)
                # # 数据集路径
                TRAIN_IMAGE_DIR = r'./RBC_Dataset'+data_folder+r'\train2017'
                VAL_IMAGE_DIR = r'./RBC_Dataset'+data_folder+r'\val2017'
                TEST_IMAGE_DIR = r'./RBC_Dataset' + data_folder + r'\test2017'
                MASK_DIR = r"./RBC_Dataset/COCO_Annotations_png"  # 所有标签的文件夹
                OUTPUT_TRAIN_JSON = r'./RBC_Dataset/'+data_folder+r'\annotations/instances_train2017.json'
                OUTPUT_VAL_JSON = r'./RBC_Dataset/'+data_folder+r'\annotations/instances_val2017.json'
                OUTPUT_TEST_JSON = r'./RBC_Dataset/'+data_folder+r'\annotations/instances_test2017.json'

                # 生成训练集和测试集的 JSON 文件
                create_coco_json(TRAIN_IMAGE_DIR, OUTPUT_TRAIN_JSON,MASK_DIR)
                create_coco_json(TEST_IMAGE_DIR, OUTPUT_TEST_JSON,MASK_DIR)
                create_coco_json(VAL_IMAGE_DIR, OUTPUT_VAL_JSON, MASK_DIR)
            ...
            # 训练数据集的根目录
            parser.add_argument('--data-path', default='RBC_Dataset'+data_folder+'/', help='dataset')
            # 检测目标类别数(不包含背景)
            ...
            parser_eval.add_argument('--data-path',
                                default=r'./RBC_Dataset'+data_folder,
            ...
            # 示例调用
            csv_file = output_folder+r'\results'+str(i)+'.csv'
            improve_folder = r"./RBC_Dataset"+data_folder+"/improve"
            train2017_folder = r"./RBC_Dataset"+data_folder+"/train2017"
